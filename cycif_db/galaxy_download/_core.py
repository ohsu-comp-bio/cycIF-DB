...
def find_markers_csv_and_quantification(his_client, history_id,
    ...
    """
    # TODO check whether history is deleted
...
def find_markers_csv_and_quantification_v2(his_client, history_id,
                                           check_naive_state=6):
    """ Find two datasets martching `markers.csv` and quantifiation.
    ...
    if not ns_dataset:
        log.warn("Error: make sure the history runs cycif workflow and is completed "
                 "successfully! %s."
                 % history_id)
        return
